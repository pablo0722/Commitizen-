...
class Check:
    ...
    def __call__(self):
        ...
        for commit in commits:
            if not Check.validate_commit_message(commit.message, pattern):
                ill_formated_commits.append(commit)
    ...
    def validate_commit_message(commit_msg: str, pattern: str) -> bool:
        ...
        for k, p in pattern.items():
            m=re.match(p, commit_msg)
            if bool(m)==False:
                out.error(f"Failed matching '{k}'. re: '{p}', msg: '{commit_msg}'")
                return False
            out.info(f"{k}: '{m.group(0)}'")
            commit_msg = commit_msg[len(m.group(0)):]
        return True
